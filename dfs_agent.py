...
class DFSAgent(Agent):
    ...
    def solve(self, level_matrix, goal, player_row, player_column):
        ...
        s = [Node(None, level_matrix, position[0], position[1], 0, None)]
        self.generated_node_count = 1
        self.maximum_node_in_memory_count = len(s)
        visited = [[False] * col_count for i in range(row_count)]
        while True:
            if self.maximum_node_in_memory_count < len(s):
                self.maximum_node_in_memory_count = len(s)
            node = s.pop()
            ...
            for i in range(len(directions)):
                ...
                if level_matrix[row][col] != "W" and not visited[row][col]:
                    s.append(Node(node, level_matrix, row, col, node.depth + 1, directions[i][0]))
                    self.generated_node_count += 1
        """
            YOUR CODE ENDS HERE
